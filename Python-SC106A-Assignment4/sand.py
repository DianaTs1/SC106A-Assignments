...
def check_move(grid, x1, y1, x2, y2):
    """
        ...
        False
        """
    # If the destination is not within the edges of the grid.
    if len(grid[0]) <= x2 or len(grid) <= y2:
        return False

    # This piece of code makes sure that one sand of the edge won't move to the other edge
    elif x2 < 0 or x2 >= len(grid[0]):
        return False

    # If sand moved diagonally
    elif x1 != x2 and y1 != y2:
        if grid[y2-1][x2] == None and grid[y2][x2] == None:
            return True
        return False

    # If sand moves only left of right
    elif x1 != x2 or y1 != y2:
        if grid[y2][x2] == None:
            return True
        return False
...
def do_gravity(grid, x, y):
    """
      ...
      [[None, None, None], [None, 's', None]]
      """
    # If (x,y) location is not sand
    if grid[y][x] != "s":
        return grid

    # To move the sand down
    elif check_move(grid, x, y, x, y + 1):
        do_move(grid, x, y, x, y + 1)
        return grid

    # To move the sand down-left
    elif check_move(grid, x, y, x - 1, y + 1):
        do_move(grid, x, y, x - 1, y + 1)
        return grid

    # To move the sand down-right
    elif check_move(grid, x, y, x + 1, y + 1):
        do_move(grid, x, y, x + 1, y + 1)
        return grid

    # if everything on the bottom is  occupied
    else:
        return grid
